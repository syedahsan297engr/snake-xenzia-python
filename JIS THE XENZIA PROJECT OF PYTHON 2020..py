...
speak("hello, how are you? You are playing JIS the xenzia made by ahssan, imran and jelaani.  press space bar to play")
########################################
...
# the line below generates a screen for a whille we have of our given width and height
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE:  # after pressing the space key
                    pygame.mixer.music.load('snake background.mp3')  # here the music will be loaded
...
def gameloop():
    ...
    while not exit_game:
        if game_over:
            ...
            if abs(snake_x - food_x) < 10 and abs(
                    snake_y - food_y) < 10:  # we use abs which give approx w   e also change it
            ...
            if head in snk_list[
                ...
                game_over = True
                pygame.mixer.music.load('game over.wav')  # here the game over sound and we define it at start
